...
describe("Verify the UserFacade", () => {
  ...
  it("Should find Donald Duck", async () => {
    try {
      const projection = { password: 0, role: 0, _id: 0 }
      const donald = { name: "Donald Duck", userName: "dd@b.dk" }
      const status = await UserFacade.getUser("dd@b.dk", projection);
  ...
  it("Should correctly validate Peter Pan's credential,s", async () => {
    try {
      const donald = await UserFacade.checkUser("pp@b.dk", "secret");
    } catch (err) {
      throw err;
    }
    finally { }
